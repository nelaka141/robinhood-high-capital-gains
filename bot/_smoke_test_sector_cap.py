...
bot/_smoke_test_blocked.py). All test symbols share an IDENTICAL close-price
series so step3_underweight_buys's real RSI/EMA math ties every Momentum_Score -> the
...
def _ctx(symbols: list, sector_groups: dict, market_values: dict = None, target_pct: float = 1.0,
         **meta_overrides) -> RunContext:
    """current_cash=$4500, no tax reserve -> base_deployable_cash = $4500 exactly. Every symbol
    shares the identical close series (tied Momentum_Score), and `target_pct` sizes each
    candidate's drift gap (target_pct% of the $10,000 account_balance) — under the top-down
    full-fill logic each candidate is filled to exactly that gap, in ranking order, until the
    $4500 runs out.
...
def test_same_sector_pair_scaled_down_to_cap() -> None:
    """X1/X2 (same group, both unheld) and Y1 (ungrouped) tie on Momentum_Score, each with a
    $1500 drift gap -> the top-down fill gives each its full $1500 out of the $4500 cash.
    X1+X2 = $3000 exceeds the 20%-of-$10,000 = $2000
    ...
    ctx = _ctx(["X1", "X2", "Y1"], sector_groups={"grp": ["X1", "X2"]}, target_pct=15.0)
...
def test_ungrouped_symbols_never_capped() -> None:
    ...
    ctx = _ctx(["A", "B", "C"], sector_groups={}, target_pct=15.0)
    allocations = step3_underweight_buys(ctx, _TiedBroker())
    for s in ["A", "B", "C"]:
        assert math.isclose(allocations[s], 1500.0, abs_tol=0.01), (
            f"no sector_groups configured -> {s} should get its full $1500 gap fill, got {allocations[s]:.2f}"
        )
    print("[sector-cap-no-groups] no sector_groups configured -> full $1500 gap fills, no scaling — OK")
...
def test_existing_holdings_alone_can_exhaust_sector_headroom() -> None:
    ...
    even starts -> zero headroom left -> any NEW planned allocation for X2 (same group)
    must scale to $0, never negative (X1's own $1500 target is already exceeded by its $2000
    holding, so its gap is 0 and it isn't allocated anything in the first place). Y1 (different
...
def test_per_group_max_percentage_override() -> None:
    """4 symbols tie on Momentum_Score, each with a $1125 drift gap -> the top-down fill gives
    each its full $1125 out of the $4500 cash
