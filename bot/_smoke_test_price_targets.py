...
1. `target_price_to_sell` (symbol -> price floor): blocks a sale of that symbol by ANY
   mechanism — Drawdown Audit, Fresh Alpha Leader Stop, blocked+forceSell liquidation, GET THE
   PROFITS, Momentum Reversal Trim, and routine/forceSell-driven Overweight trims alike — until
...
2. `target_price_to_buy` (symbol -> price ceiling): blocks a buy of that symbol by ANY
   mechanism (Underweight allocation, Alpha Multiplier allocation, profit-sell repurchase) AND
   removes it from Alpha Leader candidacy entirely, irrespective of Momentum_Score, until
   `current_price` drops to/below the ceiling.

Pure logic tests against RunContext/steps functions directly (same style as
bot/_smoke_test_wash_sale.py and bot/_smoke_test_alpha_reserve.py).
...
def test_sell_target_blocks_fresh_alpha_leader_stop() -> None:
    """ALPHA was bought as Alpha Leader 1 day ago at $100; now $80 (-20%, clears the 15% fresh
    stop threshold), but its target_price_to_sell ($90) hasn't been crossed -> no liquidation."""
    cfg = _cfg(["ALPHA"], target_price_to_sell={"ALPHA": 90.0})
    ctx = RunContext(current_date=date(2026, 8, 14), config=cfg, account_number="TEST")
    ctx.price_state = {"ALPHA": AssetPriceState(
        lastAlphaLeaderBuyPrice=100.0, lastAlphaLeaderBuyDate="2026-08-13",
    )}
    positions = {"ALPHA": Position(symbol="ALPHA", quantity=10.0, avg_cost_basis=100.0)}
    quotes = {"ALPHA": Quote(symbol="ALPHA", last_trade_price=80.0)}
    steps.step1_fetch_state(ctx, _Broker(positions, quotes), repo_dir="/nonexistent")
    assert ctx.fresh_alpha_leader_liquidations == [], ctx.fresh_alpha_leader_liquidations
    assert any(
        s.symbol == "ALPHA" and "target_price_to_sell" in s.reason
        and s.would_be_action == "Fresh Alpha Leader Stop liquidation"
        for s in ctx.skipped
    ), ctx.skipped
    print("[sell-target-blocks-fresh-alpha-stop] ALPHA would breach the fresh stop but target_price_to_sell blocks it — OK")
...
def test_sell_target_blocks_overweight_trim() -> None:
    """TRIM is Overweight, profit-gated, and would normally be sized to cover a harvest shortfall
    — but target_price_to_sell ($120) hasn't been crossed at the current $102 price."""
    cfg = _cfg(["TRIM"], target_price_to_sell={"TRIM": 120.0})
    ctx = RunContext(current_date=date(2026, 8, 14), config=cfg, account_number="TEST")
    ctx.alpha_leader = None
    ctx.positions = {"TRIM": Position(symbol="TRIM", quantity=10.0, avg_cost_basis=100.0)}
    ctx.quotes = {"TRIM": Quote(symbol="TRIM", last_trade_price=102.0)}
    ctx.momentum_scores = {}
    ctx.account_balance = 10_000.0
    ctx.drift_results = {
        "TRIM": DriftResult("TRIM", 60, 60, 50, 10, 10, 1, market_value=1020.0),
    }
    ctx.harvest_needed_dollars = 500.0
    lots = {"TRIM": [TaxLot("lot1", quantity=10.0, cost_per_share=100.0, open_date=date(2026, 1, 1), is_selectable=True)]}
    steps.step4_profit_taking(ctx, _Broker(lots=lots))
    assert ctx.overweight_trims == [], ctx.overweight_trims
    assert any(s.symbol == "TRIM" and "target_price_to_sell" in s.reason for s in ctx.skipped), ctx.skipped
    print("[sell-target-blocks-overweight-trim] TRIM would be harvested but target_price_to_sell blocks it — OK")


# ------------------------------------------------------------------------------------------
# target_price_to_buy — Underweight allocation + Alpha Leader candidacy
# ------------------------------------------------------------------------------------------
...
def test_buy_target_blocks_underweight_allocation() -> None:
    ...
    allocations = steps.step3_alpha_leader(ctx, _Step3Broker({"CHASE": 0.6}))
...
def test_buy_target_excludes_alpha_leader_candidacy() -> None:
    """TOP has the highest Momentum_Score but its price ($150) is above its target_price_to_buy
    ($120) -> it must be skipped by the Alpha Leader cascade (alpha_buy_guarded_symbols),
    irrespective of its score, and MID (next in ranking) should act instead."""
    ...
    ctx.drift_results = {
        "TOP": DriftResult("TOP", 50, 50, 50, 50, 0, 1, market_value=1000.0),
        "MID": DriftResult("MID", 25, 25, 25, 25, 0, 1, market_value=500.0),
    }
    broker = _Step3Broker({"TOP": 0.6, "MID": 0.0})
    steps.step2_guardrails(ctx, broker)
    assert "TOP" in ctx.alpha_buy_guarded_symbols, (
        f"TOP's price is above its target_price_to_buy -> must be excluded from Alpha Leader candidacy, "
        f"got {ctx.alpha_buy_guarded_symbols}"
    )
    steps.step3_alpha_leader(ctx, broker)
    assert ctx.top_momentum_symbol == "TOP", "TOP should still be the highest-scoring symbol"
    assert ctx.alpha_leader == "MID", f"TOP must be skipped for missing its buy target, expected MID, got {ctx.alpha_leader}"
    print("[buy-target-excludes-alpha-candidacy] TOP has the top score but misses its buy target -> "
          f"MID acts as Alpha Leader instead — OK")
...
def test_buy_target_allows_once_price_at_or_below() -> None:
    ...
    the buy-target guard no longer applies and TOP wins Alpha Leader normally."""
    cfg = _cfg(["TOP", "MID"], target_price_to_buy={"TOP": 120.0})
    ctx = RunContext(current_date=date(2026, 8, 14), config=cfg, account_number="TEST")
    ctx.current_cash = 1000.0
    ctx.tax_reserve = 0.0
    ...
    broker = _Step3Broker({"TOP": 0.6, "MID": 0.0})
    steps.step2_guardrails(ctx, broker)
    assert "TOP" not in ctx.alpha_buy_guarded_symbols, ctx.alpha_buy_guarded_symbols
    steps.step3_alpha_leader(ctx, broker)
    assert ctx.alpha_leader == "TOP", f"TOP is at/below its target_price_to_buy -> should win normally, got {ctx.alpha_leader}"
    print("[buy-target-allows-at-target] TOP's price at/below target_price_to_buy -> wins Alpha Leader normally — OK")
...
def main() -> None:
    ...
    test_sell_target_blocks_fresh_alpha_leader_stop()
    test_sell_target_blocks_blocked_forcesell_liquidation()
    test_sell_target_blocks_gtp()
    test_sell_target_blocks_overweight_trim()
    test_buy_target_blocks_underweight_allocation()
    test_buy_target_excludes_alpha_leader_candidacy()
